...
track_plot <- ggplot(corpus, aes(x = energy, y = danceability, color = valence, text = track_info, alpha = 0.8)) +
  geom_point() +
  facet_wrap(~artist) +
  labs(title = "Danceability vs Energy colored by valence per artist", # Add your title here
       color = "Valence") + # Title for the color scale
  theme_minimal() +
...
```{r chroma_lowest, echo=FALSE, fig.width=11, fig.height=8}
lowest |>
...
The visual on the left displays the chromatic analysis on the **least** energetic and danceable track in this corpus. We see a wide diversity of pitch classes and magnitude throughout the whole song, typical for a jazz song. When comparing the analysis with the **most** energetic and danceable track we see a difference in the diversity of pitch class occurances, where the Miles Davis track has a significant higher diversity in pitch class and a larger range of magnitude per pitch class.


### Most energetic & danceable track  
```{r chroma_highest, echo=FALSE, fig.width=11, fig.height=8}
...
The visual on the left displays the chromatic analysis on the **most** energetic and danceable track in this corpus. We see a smaller diversity of pitch classes throughout the whole song, but a bigger magnitude for most of the pitch classes. Upon listening it became clear why, and I highly recommend you to also take a quick listen to the song. Throughout the whole song, Daft Punk used a synthesizer playing 1 note at a time, but with a lot of resonance. By controlling the amount of resonance, harmonic frequencies were created. There were however so much harmonics that they covered the all pitch classes, resulting in a small diversity of pitch class occurences in the chroma analysis. When comparing the analysis with the **least** energetic and danceable track we see a difference in the diversity of pitch class occurences, where Homework from Daft Punk has a significant lower diversity in pitch class and a smaller range of magnitude. 